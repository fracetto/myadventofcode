...
def getseq(filename):
    with open(filename, "r") as reffile:
        ...
        """ Algo : faire tant que tous les nombres trouvés dans 1 ne sont pas tous parcourus 
            marquer les cellules adjacentes en ayant une parcours optimisé
        """
        for origin, vector_number in board._get_numbers_list().items():
            ...
            while (not adjacent_cells_marked and not found_symbol):
                for adj_cell in adjacent_cells:
                    found_symbol = adj_cell.value != "." and not adj_cell.value.isnumeric()
                    if found_symbol:
                        # faire le calcul
                        result += vector_number._get_value_number()
                        break
